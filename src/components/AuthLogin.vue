...
import { ref } from 'vue';
import Google from '@assets/auth/social-google.svg';
...
async function validate(_values: any, { setErrors }: any) {
  ...
  if(await authStore.login(username.value, password.value).catch((error: any) => setErrors({ apiError: error }))){
...
<template>
  <v-btn block color="primary" variant="outlined" class="text-lightText googleBtn mt-2">
  ...
  <v-row>
    <v-col class="d-flex align-center">
      ...
      <v-btn variant="outlined" class="orbtn mt-0 mb-0" rounded="md" size="small">OR</v-btn>
      <v-divider class="custom-devider" />
    </v-col>
  </v-row>
